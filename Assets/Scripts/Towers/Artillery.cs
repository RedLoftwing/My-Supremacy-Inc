...
public class Artillery : FixedTargetTower
{
    ...
    private IEnumerator FireWeapon()
    {
        ...
        if (!gameStateScript.isInterWave)
        {
            ...
            explosion = Instantiate(explosionPrefab, currentTarget, Quaternion.identity);
            weaponFire.Play();
            //Gathers all colliders within the "explosions" radius...
            int numColliders = Physics.OverlapSphereNonAlloc(explosion.transform.position, explosionRadius, colliders, enemyLayerMask);
            //IF the number of colliders is greater than 0...then go through each collider, and find the enemy component.
            if (numColliders > 0)
            {
                for (int i = 0; i < numColliders; i++)
                {
                    var enemyToDamage = colliders[i].GetComponent<Enemy>();
                    ...
                    if (enemyToDamage != null)
                    {
                        for (int j = 0; j < validTargets.Length; j++)
                        {
                            ...
                            if(enemyToDamage.CompareTag(validTargets[j]))
                            {
